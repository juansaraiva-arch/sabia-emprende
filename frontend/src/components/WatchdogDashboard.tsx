...
import type { SemaforoItem, SancionCategory, RecargoResult } from "@/lib/mupaEngine";
...
export default function WatchdogDashboard() {
  ...
  // MUPA: Verificar si Declaracion Jurada existe en Boveda KYC
  const [declaracionEnBoveda, setDeclaracionEnBoveda] = useState(false);

  React.useEffect(() => {
    // Check sync events + localStorage for declaracion_mupa document
    const checkBoveda = () => {
      const events = getDocSyncEvents();
      const hasDeclaracion = events.some((e) => e.category === "declaracion_mupa");
      // Also check step documents localStorage
      if (typeof window !== "undefined") {
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
        ...
        <div className="p-5 space-y-5">
          ...
          {/* ── SELECTOR DE PUBLICIDAD (Diagnostico Estrategico) ── */}
            ...
            <label className="flex items-start gap-3 cursor-pointer group">
              ...
              <div>
                ...
                <p className="text-[10px] text-amber-600 mt-0.5">
                  Si tu negocio tiene rotulacion exterior, vallas o vehiculos con publicidad, necesitas un permiso del MUPA. Al marcar esta casilla activaras alertas de precaucion sobre sanciones administrativas por publicidad no declarada.
                </p>
              </div>
            </label>
            {hasPublicidad && (
              <div className="mt-2 ml-7 p-2.5 rounded-lg bg-red-50 border border-red-200">
                <div className="flex items-center gap-2">
                  <AlertTriangle size={14} className="text-red-500" />
                  <span className="text-[11px] font-bold text-red-700">Alerta de Precaucion</span>
                </div>
                <p className="text-[10px] text-red-600 mt-1">
                  Riesgo de sanciones: Publicidad no declarada ($10 a $500 + remocion), rotulos en servidumbre publica ($50 a $1,000). Verifica que tu permiso de publicidad este vigente en la Boveda KYC.
                </p>
              </div>
            )}
          </div>
    ...
    <details className="rounded-xl border border-slate-200 overflow-hidden">
      ...
      <div className="p-4 bg-white space-y-4">
        {MATRIZ_SANCIONES.map((cat) => {
          // Highlight publicidad if user has letreros
